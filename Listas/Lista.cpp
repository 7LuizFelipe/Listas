...
bool Lista_Inserir_Ordenada(TipoDado Elemento) {
    // Transformar a string para min�sculas
    std::transform(Elemento.begin(), Elemento.end(), Elemento.begin(), ::tolower);

    // Verificar se o elemento j� existe na lista
    TipoCelula *Atual = Frente;
    while (Atual != NULL) {
        if (Atual->Item == Elemento) {
            printf("Elemento '%s' j� est� na lista!\n", Elemento.c_str());
            return false; // Elemento j� existe
        }
        Atual = Atual->Prox;
    }
